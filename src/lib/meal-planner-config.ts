...
const DINNER_NOTES = [
  "How to use this Dinner planner",
  "1. Each row is one component of the plate. Pick an Item from its dropdown and type the RAW/DRY grams.",
  "2. Calories + macros auto-fill; TOTAL PLATE sums the whole dinner. Leave an item blank to skip a slot.",
  "3. Keep it light: dinner fills the calories LEFT after breakfast + lunch. Eat protein + veg first, the little grain LAST, walk after.",
  "",
  "Dinner = the LIGHT, remaining-quota meal",
  "- Your day is ~1900 kcal; dinner is whatever is LEFT after breakfast + lunch.",
  "- After logging breakfast + lunch, set the 'Dinner target' to your actual remaining number and build to it.",
  "- If lunch ran high, make dinner smaller — trim chicken / grain / oil first.",
  "",
  "Vegetables — solid & non-starchy only",
  "- Three veg slots to build a stir-fry mix (capsicum, carrot, French beans, zucchini, broccoli, mushroom, gourds, etc.).",
  "- No curry paste and no leafy slot — just solid vegetables.",
  "- Starchy veg (potato, sweet potato, yam, colocasia, plantain) are intentionally NOT here.",
  "- Beetroot/carrot are solid but a little sugary — fine in moderation.",
  "",
  "Grain — low and boil-in-water only",
  "- Broken wheat (dalia) or a millet, soaked and boiled in water. No roti, no rice. Keep it small (default ~25g dry); clear the slot to skip it.",
  "",
  "Protein & dairy",
  "- Chicken / fish / egg / paneer. Buttermilk (chaas) is the default dairy; curd or Greek yogurt are swaps.",
  "",
  "Raw vs cooked",
  "- All numbers are per 100 g of RAW / DRY ingredient — enter RAW weights.",
  "- Chicken/fish: weigh raw (loses ~25% cooking). Grain: weigh DRY. Eat the little grain LAST, then a short walk.",
  "",
  "Note: a planning tool, not medical advice. Keep medication and clinical decisions with your doctor.",
].join("\n");

const DINNER: MealPlannerConfig = {
  title: "Dinner",
  categories: ["Protein", "Veg", "Grain", "Dairy", "Spices", "Cooking Fat"],
  slots: [
    { id: "protein", label: "Protein (main)", category: "Protein" },
    { id: "veg1", label: "Vegetable 1", category: "Veg" },
    { id: "veg2", label: "Vegetable 2", category: "Veg" },
    { id: "veg3", label: "Vegetable 3", category: "Veg" },
    { id: "grain", label: "Grain (low)", category: "Grain" },
    { id: "dairy", label: "Dairy / buttermilk", category: "Dairy" },
    { id: "spices", label: "Spices", category: "Spices" },
    { id: "fat", label: "Cooking Fat", category: "Cooking Fat" },
  ],
  defaults: {
    target: { kcal: 480, protein: 42, carbs: 40, fiber: 12 },
    notes: DINNER_NOTES,
    foods: [
      F("Protein", "Chicken breast, skinless (raw)", 120, 22.5, 0, 0, "Grill / tandoori / stir-fry, low oil", "loses ~25% wt on cooking", "Leanest, highest protein"),
      F("Protein", "Chicken curry-cut, skinless (raw)", 150, 19, 0, 0, "Grill / light curry", "loses ~25% wt on cooking", "Tastier, a bit fattier"),
      F("Protein", "Boneless fish fillet, lean (seer/tilapia) (raw)", 100, 20, 0, 0, "Pan-sear / bake / grill in light oil", "150-180g raw", "Lean, low-GI"),
      F("Protein", "Salmon (raw)", 208, 20, 0, 0, "Bake / pan in light oil", "150g raw", "Omega-3 rich, higher fat"),
      F("Protein", "Egg, whole (raw)", 143, 13, 1, 0, "Bhurji / boiled / omelette, low oil", "1 egg ~ 50g"),
      F("Protein", "Paneer (full-fat)", 296, 18, 6, 0, "Grilled / bhurji, low oil", "100g block", "Veg-day option"),
      F("Protein", "Paneer (low-fat)", 206, 18, 6, 0, "Grilled / bhurji, low oil", "100g block", "Same protein, fewer calories"),
      F("Veg", "Capsicum / Bell pepper", 31, 1, 6, 2.1, "Stir-fry / roast", "1 cup ~ 100g", "Your staple"),
      F("Veg", "Carrot", 41, 0.9, 10, 2.8, "Stir-fry / roast", "1 cup ~ 100g", "Your staple"),
      F("Veg", "French beans", 31, 1.8, 7, 3.4, "Stir-fry / saute", "1 cup ~ 100g", "Your staple"),
      F("Veg", "Zucchini", 17, 1.2, 3.1, 1, "Stir-fry / roast", "1 cup ~ 100g", "Your staple, very low cal"),
      F("Veg", "Broccoli", 34, 2.8, 7, 2.6, "Stir-fry / steam", "1 cup ~ 100g", "High fibre"),
      F("Veg", "Cauliflower", 25, 1.9, 5, 2, "Stir-fry / roast", "1 cup ~ 100g"),
      F("Veg", "Cabbage", 25, 1.3, 6, 2.5, "Stir-fry", "1 cup ~ 100g"),
      F("Veg", "Mushrooms (button)", 22, 3.1, 3.3, 1, "Stir-fry / saute", "1 cup ~ 100g", "Low cal, savoury protein"),
      F("Veg", "Cucumber", 15, 0.7, 3.6, 0.5, "Raw / salad", "1 cup ~ 100g", "Very low cal"),
      F("Veg", "Bottle gourd (sorakaya)", 14, 0.6, 3.4, 0.5, "Stir-fry / saute", "1 cup ~ 100g", "Very low cal"),
      F("Veg", "Ridge gourd (beerakaya)", 20, 1.2, 4.4, 1.5, "Stir-fry / saute", "1 cup ~ 100g"),
      F("Veg", "Brinjal (vankaya)", 25, 1, 6, 3, "Roast / stir-fry, light oil", "1 cup ~ 100g"),
      F("Veg", "Bhindi / Okra", 33, 1.9, 7, 3.2, "Stir-fry, minimal oil", "1 cup ~ 100g"),
      F("Veg", "Cluster beans (goru chikkudu)", 32, 3.2, 11, 3.2, "Stir-fry", "1 cup ~ 100g", "High fibre"),
      F("Veg", "Broad / flat beans (chikkudukaya)", 47, 2.5, 9, 3.8, "Stir-fry", "1 cup ~ 100g"),
      F("Veg", "Snake gourd", 18, 0.5, 3.3, 0.8, "Stir-fry / saute", "1 cup ~ 100g"),
      F("Veg", "Ash gourd", 13, 0.4, 3, 2.9, "Stir-fry / saute", "1 cup ~ 100g", "Very low cal"),
      F("Veg", "Tindora (dondakaya)", 18, 1.2, 3.1, 1.6, "Stir-fry", "1 cup ~ 100g", "Low-GI"),
      F("Veg", "Bitter gourd (kakarakaya)", 17, 1, 3.7, 2.8, "Stir-fry (often solo)", "1 cup ~ 100g", "Good for blood sugar"),
      F("Veg", "Beetroot", 43, 1.6, 10, 2.8, "Stir-fry / steam", "1 cup ~ 100g", "Solid but higher sugar - moderate"),
      F("Veg", "Radish (mullangi)", 16, 0.7, 3.4, 1.6, "Stir-fry / salad", "1 cup ~ 100g", "Low cal"),
      F("Veg", "Onion", 40, 1.1, 9, 1.7, "Saute into the stir-fry", "1 medium ~ 100g", "Solid pieces (not paste)"),
      F("Veg", "Tomato", 18, 0.9, 3.9, 1.2, "Into the stir-fry", "1 medium ~ 80g", "Solid pieces (not paste)"),
      F("Grain", "Broken wheat (dalia / cracked wheat) - dry", 342, 12, 70, 9, "Soak & boil in water (no roti). Keep LOW + walk after", "20-30g dry", "Very low quantity for dinner"),
      F("Grain", "Foxtail millet (korralu) - dry", 351, 12, 63, 8, "Soak & boil in water like rice. Keep LOW + walk", "20-30g dry", "Low-GI"),
      F("Grain", "Little millet (samalu) - dry", 346, 7.7, 67, 7.7, "Soak & boil in water. Keep LOW + walk", "20-30g dry"),
      F("Grain", "Kodo millet (arikelu) - dry", 353, 9.8, 66, 6, "Soak & boil in water. Keep LOW + walk", "20-30g dry"),
      F("Grain", "Barnyard millet (udalu) - dry", 342, 11, 65, 10, "Soak & boil in water. Keep LOW + walk", "20-30g dry", "Lowest-GI, high fibre"),
      F("Dairy", "Buttermilk (chaas)", 25, 1.3, 2.8, 0, "Plain / spiced, chilled", "1 glass ~ 200ml", "Light, filling - you like it"),
      F("Dairy", "Curd (toned)", 60, 3.4, 4.7, 0, "Plain, chilled", "1 katori ~ 150g", "Use toned"),
      F("Dairy", "Greek yogurt (plain)", 60, 10, 3.6, 0, "Plain, chilled", "1 cup ~ 200g", "High protein"),
      F("Spices", "Spice powder mix (turmeric+red chili+cumin+coriander+garam masala)", 335, 13, 52, 27, "Dry powder mix for the stir-fry", "~5g for a light dinner", "Turmeric + Red chilli + Cumin + Coriander + Garam masala"),
      F("Cooking Fat", "Groundnut oil", 884, 0, 0, 0, "For the stir-fry - keep it light", "1 tsp ~ 5g"),
      F("Cooking Fat", "Olive oil", 884, 0, 0, 0, "Low-heat / finishing", "1 tsp ~ 5g"),
      F("Cooking Fat", "Mustard oil", 884, 0, 0, 0, "For the stir-fry", "1 tsp ~ 5g"),
      F("Cooking Fat", "Ghee", 900, 0, 0, 0, "Small amount only", "1 tsp ~ 5g"),
    ],
  },
};
